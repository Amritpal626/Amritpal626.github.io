...
function searchVideo()
{
    ...
    let keyword = document.getElementById("txt-search").value
    let count = 0
    document.querySelector("#results").innerHTML=``
    if(keyword === ``)
    {
    ...
    else
    {
      for(let i = 0; i < videosList.length; i++)
      {
        if(videosList[i].description.search(keyword) != -1)
        {
          document.querySelector("#results").innerHTML+=generateRowHTML(videosList[i]) 
          count++
        }
      ...
      if(count === 0)
      {
        document.querySelector("#results").innerText=`No videos found with the keyword ${keyword}`
      }
    }
